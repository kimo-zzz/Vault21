...
{
	DX11::InputHook::UnloadHook(window);
...
void DXGICapture::DoPresent(IDXGISwapChain* swap, UINT syncInterval, UINT flags)
{
	...
	std::call_once(is_init, [&]() {
		if (SUCCEEDED(swap->GetDevice(__uuidof(pDevice), reinterpret_cast<void**>(&pDevice))))
		{
			...
			if (SUCCEEDED(swap->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&pRenderTargetTexture)))
			{
				...
				//render.m_hwnd = window;
				render.init11(pDevice, pContext);
	...
	render.begin_draw11();//begin for draw rende.drawline.... and etc
	...
	render.end_draw11();//end for draw render.drawline.... and etc. NO NEED FOR THIS BECAUSE AFTER DX11::MENU RENDERS, IT HAS END_DRAW
}
