	...
	*/
	if (pExceptionInfo->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION)
	{
		...
		for (HookEntries hs : hookEntries)
		{
			...
			else if ((hs.addressToHookMbiEnd <= pExceptionInfo->ContextRecord->XIP) &&
					(hs.addressToHookMbiEnd + 0x10 >= pExceptionInfo->ContextRecord->XIP) &&
					(pExceptionInfo->ExceptionRecord->ExceptionInformation[0] == 8)
				) {
				int offset = pExceptionInfo->ContextRecord->XIP - hs.addressToHookMbiEnd;
